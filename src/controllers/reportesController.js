...
reportesController.generateReport = async (req, res, next) => {
  ...
  let status = false, errors = 'Hubo un error, intente más tarde.';
  const idUser = req.user.idUser;
  if (type == 1) {
    // day
    ...
    const content = await reportes.toDay(idUser, date, date, date);
    if(!content) errors = 'Al parecer ese día no se trabajó.';
    else{
      try {
        await pdf
          .create(content, {
    ...
    const content = await reportes.toDay(idUser, date, dateFirst, dateLast, true);
